...
$PAGE->set_title(get_string('pluginname', 'auth_mini'));
$PAGE->set_heading(get_string('signup_view', 'auth_mini'));
...
if (!empty($availableenrolids)) {
    ...
    foreach ($availableenrolids as $enrolid) {
        ...
        if ($plugin->enrolstartdate > 0 && $plugin->enrolenddate > 0) {
            ...
            $successoutput = get_string('signup_view_message_basic_dates', 'auth_mini', $data);

        } else if ($plugin->enrolstartdate > 0 && $plugin->enrolenddate == 0) {
            // The course only has a start date set.
            $successoutput = get_string('signup_view_message_basic_dates_startonly', 'auth_mini', $data);

        } else if ($plugin->enrolstartdate == 0 && $plugin->enrolenddate > 0) {
            // The course only has a start date set.
            $successoutput = get_string('signup_view_message_basic_dates_endonly', 'auth_mini', $data);

        } else {
            // The course has no date restrictions.
            $successoutput = get_string('signup_view_message_basic', 'auth_mini', $data);
    ...
    $failure = get_string('signup_failure', 'auth_mini', $data);
