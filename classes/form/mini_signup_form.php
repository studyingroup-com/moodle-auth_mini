...
class enrolkey_signup_form extends \login_signup_form {
    ...
    public function definition() {
        global $CFG;
        global $PAGE;
        $PAGE->requires->js('/auth/mini/js/mini_signup_form.js');
        ...
        if(get_config('auth_mini', 'tokenrequired')) {
            $element = $mform->createElement('text', 'signup_token', get_string('signup_field_title', 'auth_mini'));
            ...
            if ($this->signup_token_required()) {
                $mform->addRule('signup_token', get_string('signup_missing', 'auth_mini'), 'required', null, 'server');
            }
        }
        ...
        if(!get_config('auth_mini', 'enabledfirstname')) {
            $mform->hideif('firstname', '');
            $mform->setDefault('firstname', "firstname");
        }
        if(!get_config('auth_mini', 'enabledlastname')) {
            $mform->hideif('lastname', '');
            $mform->setDefault('lastname', "lastname");
        }
        if(!get_config('auth_mini', 'enabledcity')) {
            $mform->removeElement('city');
        }
        if(!get_config('auth_mini', 'enabledcountry')) {
        ...
        if(!get_config('auth_mini', 'enablefirstname') ||
            !get_config('auth_mini', 'requiredfirstname')){
            unset($mform->_rules['firstname'], $mform->_errors['firstname'],
                $mform->_required['4']);
        }
        if(!get_config('auth_mini', 'enablelastname') ||
            !get_config('auth_mini', 'requiredlastname')){
            unset($mform->_rules['lastname'], $mform->_errors['lastname'],
                $mform->_required['5']);
        }
        if(get_config('auth_mini', 'requiredcity')) {
            $mform->addRule('city', get_string('missingcity', 'auth_mini'), 'required', null, 'client');
        }
        if(get_config('auth_mini', 'requiredcountry')) {
            $mform->addRule('country', get_string('missingcountry', 'auth_mini'), 'required', null, 'client');
        }
    ...
    public function validation($data, $files) {
        ...
        if (!empty($token)) {
            ...
            if (!$selfenrolinstance) {
                $errors['signup_token'] = get_string('signup_token_invalid', 'auth_mini');
            }
        }
    ...
    public function signup_token_required() {
        return get_config('auth_mini', 'tokenrequired');
    }
    ...
    public function signup_captcha_enabled() {
        global $CFG;
        return !empty($CFG->recaptchapublickey) && !empty($CFG->recaptchaprivatekey) && get_config('auth_mini', 'recaptcha');
    }
